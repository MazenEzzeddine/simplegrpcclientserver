...
public class MessageServer {
    ...
    private static class MessageService extends MessageServiceGrpc.MessageServiceImplBase {
        public void inbox(Username request,
                          StreamObserver<InboxReply> responseObserver) {
            InboxReply reply = InboxReply.newBuilder().addMessages(
                    Message.newBuilder()
                            .setId("1234")
                            .setFromUser("Paul")
                            .setToUser("Veronica")
    ...
    public static void main(String[] args) throws Exception {
        MessageServer server = new MessageServer(5002);
        server.start();
